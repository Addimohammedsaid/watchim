...
export class MovieService {
  ...
  get lastEpisodes() {
    return this._db.collection("episodes", (ref) => {
      let q = ref.orderBy("index", "desc").limit(12);
      return q;
    });
  }

  paginate(index: number, pageSize: number) {
    console.log(index, pageSize);
    return this._db.collection("episodes", (ref) => {
      let q = ref.orderBy("index", "desc").startAt(index).limit(pageSize);
      return q;
    });
  }
  ...
  addEpisode(episode) {
    ...
    return this.getLastEpisodeIndex()
      .get()
      .forEach((e) => {
        let index = e.docs[0].data()["index"] + 1;
  ...
  getLastEpisodeIndex() {
    return this._db.collection("episodes", (ref) => {
      let q = ref.orderBy("index", "asc").limitToLast(1);
      return q;
    });
  }
  ...
  // DELETE
  deleteMovie(key) {
    return this._db.collection("movies").doc(key).delete();
  }
