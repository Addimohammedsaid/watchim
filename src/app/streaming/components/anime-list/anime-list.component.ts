...
export class AnimeListComponent implements OnInit {
 //
  ...
  imageUrl  = "https://www.dailynews.com/wp-content/uploads/2020/02/LDN-L-MYHERO-0228-10-1.jpg";
  title = "Find Your Anime here !"

  constructor(private route: ActivatedRoute, private _moviesService: MovieService, private _auth: AuthService)
  {    
    ...
    .pipe(
      map((changes) =>
        changes.map((c) => ({ key: c.payload.doc.id, ...c.payload.doc.data() as Anime } ))
      )
    )
    .pipe(
      switchMap((animeList) => {        
    ...
    .subscribe((params) => {
      ...
      this.filteredAnime = this.category 
        ...
        this.letter ? this.animeList.filter((e) => e.name[0].toUpperCase() == this.letter)
        : this.animeList;
    });
